...
import { update } from '@/actions/App/Http/Controllers/OnboardingController';
...
<template>
    <Head :title="$t('fitness.onboarding_title')" />

    <div class="flex min-h-screen items-center justify-center bg-background px-4 py-12">
        <div class="w-full max-w-lg space-y-4">
            <div class="text-center">
                <h1 class="text-2xl font-bold">{{ $t('fitness.onboarding_title') }}</h1>
                <p class="mt-1 text-sm text-muted-foreground">{{ $t('fitness.onboarding_desc') }}</p>
            </div>
            ...
            <form @submit.prevent="submit" class="flex flex-col gap-3">
                    ...
                    <CardContent class="pt-4 pb-4 space-y-3">
                        ...
                        <template v-if="estimatedCalories">
                            ...
                            <div class="flex items-center gap-3">
                                ...
                                <div class="flex-1">
                                    <p class="text-xs text-muted-foreground">{{ $t('fitness.estimated_goal') }}</p>
                ...
                <Button type="submit" class="w-full" :disabled="form.processing">
                    {{ form.processing ? $t('fitness.saving') : $t('fitness.save_continue') }}
                </Button>
            </form>
        </div>
    </div>
